    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            Path { path in
                ...
                path.addQuadCurve(
                    to: CGPoint(x: nearLine2, y: middle),
                    control: CGPoint(x: nearLine2, y: nearLine2)
                )
                path.addQuadCurve(
                    to: CGPoint(x: middle, y: farLine2),
                    control: CGPoint(x: nearLine2, y: farLine2)
                )
                path.addQuadCurve(
                    to: CGPoint(x: farLine2, y: middle),
                    control: CGPoint(x: farLine2, y: farLine2)
                )
                path.addQuadCurve(
                    to: CGPoint(x: middle, y: nearLine2),
                    control: CGPoint(x: farLine2, y: nearLine2)
                )
            }
            .fill(Color("grayOwl"))
